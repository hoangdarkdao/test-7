    ...
    def get_prompt_e1(cls, task_prompt: str, indivs: List[Function], template_function: Function):
        ...
        for i, indi in enumerate(indivs):
            ...
            indivs_prompt += (
                ...
                f'{indi.algorithm}\n{str(indi)}\n'
                f'Objective value and runtime: {cls._format_score(indi.score)}\n'
            )
...
{indivs_prompt}
Please create a new algorithm that has a totally different form from the given algorithms, with distinct structures, flows, or approaches. The new algorithm should focus on balancing both the problem’s objective function and runtime trade-off.
1. Firstly, describe the design idea and main steps of your algorithm in one sentence. The description must be inside within boxed {{}}.
2. Secondly, implement the idea in the following Python function:
{str(temp_func)}
Do not give additional explanations.'''
        return prompt_content
    ...
    @classmethod
    def get_prompt_runtime(cls, task_prompt: str, indivs: List[Function], template_function: Function):
        for indi in indivs:
            assert hasattr(indi, 'algorithm')
...
I have {len(indivs)} existing algorithms with their codes as follows:
{indivs_prompt}
Please create a new algorithm that has a similar form to the No.{len(indivs)} algorithm and is inspired by the No.{1} algorithm, focusing on minimizing runtime (computational efficiency).
...
'''
    ...
    @classmethod
    def get_prompt_objective(cls, task_prompt: str, indivs: List[Function], template_function: Function):
        for indi in indivs:
            assert hasattr(indi, 'algorithm')
...
I have {len(indivs)} existing algorithms with their codes as follows:
{indivs_prompt}
Please create a new algorithm that has a similar form to the No.{len(indivs)} algorithm and is inspired by the No.{1} algorithm, focusing on improving the problem’s objective function.
1. Firstly, list the common ideas in the No.{1} algorithm that may give good performances.  
2. Secondly, based on the common idea, describe the design idea based on the No.{len(indivs)} algorithm and main steps of your algorithm in one sentence. The description must be inside within boxed {{}}.  
3. Thirdly, implement the idea in the following Python function:  
{str(temp_func)}  
...
'''
    ...
    @classmethod
    def get_prompt_both(cls, task_prompt: str, indivs: List[Function], template_function: Function):
        for indi in indivs:
            assert hasattr(indi, 'algorithm')
...
I have {len(indivs)} existing algorithms with their codes as follows:
{indivs_prompt}
Please create a new algorithm that explores novel trade-offs beyond the existing solutions and aims to expand the current Pareto front.  
You may take structural inspiration from the No.{1} and No.{len(indivs)} algorithms, but your main goal is to discover new design ideas that provide better objective values, better runtime, or both.

1. Firstly, list the common ideas in the No.{1} algorithm that may contribute to good performance.  
2. Secondly, based on these common ideas, describe the new design idea inspired by the No.{len(indivs)} algorithm in one sentence. The description must be enclosed within boxed {{}}.  
3. Thirdly, implement the idea in the following Python function:  
{str(temp_func)}  
...
I have {len(indivs)} existing algorithms with their codes as follows:
{indivs_prompt}
Please help me create a new algorithm that is inspired by all the above algorithms, focusing on balancing both the problem’s objective function and runtime trade-off.
