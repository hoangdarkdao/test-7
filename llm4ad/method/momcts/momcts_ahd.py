...
FOCUS_AREA = [
    "improving the problem’s objective function",
    "minimizing runtime (computational efficiency)",
    "balancing both the problem’s objective function and runtime trade-off"
]
...
class MOMCTS_AHD:
    ...
    def _try_fix_func_via_llm(self, broken_code: str, error_msg: str | None, tries: int = 2):
        ...
        for _ in range(tries):
            repair_prompt = MOMCTSPrompt.get_prompt_fix_code(
    ...
    def expand(self, mcts: MCTS, node_set: list[MCTSNode], cur_node: MCTSNode, option: str):
        ...
        if option == 's1':
            ...
            while i < 3:
                ...
                while True:
                    ...
                    if now_indiv != cur_node.individual:
                        break
                prompt = MOMCTSPrompt.get_prompt_runtime(self._task_description_str, [now_indiv, cur_node.individual],
                                                self._function_to_evolve)
                func = self._sample_evaluate_register(prompt, func_only=True)
                if func is False:
                    is_valid_func = False
            ...
            while i < 3:
                ...
                while True:
                    ...
                    if now_indiv != cur_node.individual:
                        break
                prompt = MOMCTSPrompt.get_prompt_objective(self._task_description_str, [now_indiv, cur_node.individual],
                                                self._function_to_evolve)
                func = self._sample_evaluate_register(prompt, func_only=True)
                if func is False:
                    is_valid_func = False
            ...
            while i < 3:
                ...
                while True:
                    ...
                    if now_indiv != cur_node.individual:
                        break
                prompt = MOMCTSPrompt.get_prompt_both(self._task_description_str, [now_indiv, cur_node.individual],
                                                self._function_to_evolve)
                func = self._sample_evaluate_register(prompt, func_only=True)
                if func is False:
                    is_valid_func = False
        ...
        if is_valid_func and np.any(func.score != float('-inf')):
            ...
            now_node = MCTSNode(func.algorithm, str(func), -1 * func.score, individual=func,
                                parent=cur_node, depth=cur_node.depth + 1, visit=1, raw_info=func)
            if option == 'e1':
    ...
    def _iteratively_init_population_root(self):
        ...
        while len(self._population.population) < self._init_pop_size:
            try:
                prompt = MOMCTSPrompt.get_prompt_e1(self._task_description_str, self._population.population,
                                                self._function_to_evolve)
                self._sample_evaluate_register(prompt)
    ...
    def run(self):
        ...
        # evolutionary search
        n_op = ['e1', 'runtime', 'objective', 'both', 'm1', 'm2', 's1']
        op_weights = [0, 1, 1, 1, 1, 1, 1]
